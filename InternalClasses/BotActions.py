...
class BotActions:
    """
    This class controls bot actions like sending e-mail notifications and
    channel broadcasts.
    ...
    async def process_messages(self, messages:list) -> None:
        ...
        for message in messages:
            print(
                " -- BotActions - Processing message: {}".format(dict(message)))
            cmd = message["command"]
            if message["command_arguments"]:
                args = message["command_arguments"].split(
                    BotSettings.config.queue_command_arguments_sep)
            else:
                args = None
    ...
    async def send_mail(self, args:list, message_content:str) -> None:
        if not args or not len(args):
            print("ERROR - BotActions.send_mail() need to receive a " +
                  "non-empty list 'args', got {}. This message " +
                  "will be ignored.".format(args))
            return
        discord_user_id, ic_sender, ic_receiver, message_title = args
        discord_user = BotSettings.bot_ref.get_user(int(discord_user_id))
        if not discord_user:
            print("ERROR - discord_user_id not found. Probably this " +
                  "user is not in contact with our bot. (On DMs and servers)")
            return
